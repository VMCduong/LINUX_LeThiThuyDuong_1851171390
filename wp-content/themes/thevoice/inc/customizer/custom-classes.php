...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class TheVoice_Separator extends WP_Customize_Control {

        public $type = 'thevoice-separator';
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Radio Image Custom Control Class.
    class TheVoice_Custom_Radio_Image_Control extends WP_Customize_Control {

        public $type = 'thevoice-radio-image';
        ...
        public function render_content() {
            ...
            <div id="input_<?php echo esc_attr($this->id); ?>" class="image thevoice-radio-image-buttenset">
 ...
 * @access public
 */
class TheVoice_Customize_Section_Upsell extends WP_Customize_Section {
...
*/
class TheVoice_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $thevoice_box_label = '';

    public $thevoice_box_add_control = '';

    public $thevoice_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->thevoice_box_label = $args['thevoice_box_label'] ;
        $this->thevoice_box_add_control = $args['thevoice_box_add_control'];
        $this->thevoice_box_add_button = $args['thevoice_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="thevoice-repeater-field-control-wrap">
            <?php
            $this->thevoice_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="thevoice-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->thevoice_box_add_button ) && $this->thevoice_box_add_button ){ ?>
            
            <button type="button" class="button thevoice-add-control-field"><?php echo esc_html( $this->thevoice_box_add_control ); ?></button>
        <?php
        }
    ...
    private function thevoice_get_fields(){
        ...
        if( is_array( $values ) ){
            ...
            foreach($values as $value){ ?>
                ...
                <li class="thevoice-repeater-field-control">

                <div class="title-rep-wrap">
                    <span class="dashicons dashicons-move twp-filter-icon"></span>
                    <h3 class="thevoice-repeater-field-title"><?php echo esc_html( $this->thevoice_box_label ); ?></h3>
                    <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
                </div>

                <div class="thevoice-repeater-fields">
                    ...
                    foreach ($fields as $key => $field) {
                        ...
                        <div class="thevoice-fields thevoice-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                            ...
                            switch ($field['type']) {
                                ...
                                case 'selector_color':
                                    ...
                                    $options = $field['options'];
                                    echo '<div class="thevoice-type-radio">';
                                ...
                                case 'upload':
                                        ...
                                        esc_html_e('No image selected', 'thevoice');
                                        echo '</div>';
                                        echo '<div class="thumbnail thumbnail-image">';
                                        echo $image;
                                        echo '</div>';
                                        echo '<div class="actions clearfix">';
                                        echo '<button type="button" class="button twp-img-delete-button align-left">'.esc_html__('Remove', 'thevoice').'</button>';
                                        echo '<button type="button" class="button twp-img-upload-button alignright">'.esc_html__('Select Image', 'thevoice').'</button>';
                    ...
                    <div class="clearfix thevoice-repeater-footer">
                        <div class="alignright">
                        <?php if( isset( $this->thevoice_box_add_button ) && $this->thevoice_box_add_button ){ ?>
                            <a class="thevoice-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'thevoice') ?>|</a> 
                        <?php } ?>
                        <a class="thevoice-repeater-field-close" href="#close"><?php esc_html_e('Close', 'thevoice') ?></a>
                        </div>
                    </div>
                </div>
                </li>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class TheVoice_Info_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                ...
                <div class="twp-info-icon">
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-move twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click hold and drag to re-order.', 'thevoice' ); ?></p>
                    </div>
                    <div class="icon-notice-wrap">
                        <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
                        <p><?php esc_html_e( 'Click to expand settings.', 'thevoice' ); ?></p>
                    </div>
                </div>
            </span>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class TheVoice_Premium_Notiece_Control extends WP_Customize_Control {
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','thevoice'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'thevoice' ); ?></span>
                    </div>
                </div>
            </span>
