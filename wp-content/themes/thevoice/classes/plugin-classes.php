<?php
if ( !class_exists('TheVoice_Getting_started') ):

    class TheVoice_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_thevoice_install_plugins', array( $this, 'thevoice_install_plugins' ) );
        ...
        public static function thevoice_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','thevoice') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','thevoice') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','thevoice') );
        ...
        public static function thevoice_recommended_plugins(){
        ...
        public function thevoice_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'thevoice' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'thevoice_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'thevoice') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->thevoice_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->thevoice_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    esc_html_e('Deactivated' , 'thevoice');
                    die();
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'thevoice');
                            die();
                        }
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=thevoice-about' );
            }
